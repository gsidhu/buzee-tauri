...
pub fn create_tables_if_not_exists(mut con: diesel::SqliteConnection) -> Result<usize, Error> {
  let _ = diesel::sql_query(DOCUMENT_TABLE_CREATE_STATEMENT.to_string()).execute(&mut con);
  let _ = diesel::sql_query(CREATE_DOCUMENT_FTS.to_string()).execute(&mut con);
  let _ = diesel::sql_query(TRIGGER_DOCUMENT_INSERT.to_string()).execute(&mut con);
  let _ = diesel::sql_query(TRIGGER_DOCUMENT_UPDATE.to_string()).execute(&mut con);
  let _ = diesel::sql_query(TRIGGER_DOCUMENT_DELETE.to_string()).execute(&mut con);
  Ok(1)
}